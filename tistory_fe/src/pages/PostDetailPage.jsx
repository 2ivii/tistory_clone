...
import { posts, comments as allComments } from "../data/posts";
...
/* ─── RelatedPost ─── */
function RelatedPost({ post, onNavigate }) {
  ...
  return (
    ...
    >
      ...
      <div style={{ padding: "14px 16px" }}>
        <div style={{ fontSize: 11, color: accent, fontWeight: 600, marginBottom: 6, textTransform: "uppercase" }}>
          {post.category}
        </div>
        <div style={{
          fontSize: 14, fontWeight: 600, color: "#191919", lineHeight: 1.4,
          display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical", overflow: "hidden",
        }}>
          {post.title}
        </div>
        <div style={{ fontSize: 12, color: "#bbb", marginTop: 8 }}>{post.date}</div>
      </div>
    </div>
  );
}
...
export default function PostDetailPage({ postId, onNavigate }) {
  ...
  const relatedPosts = (post.relatedIds || []).map(id => posts.find(p => p.id === id)).filter(Boolean);
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#f8f7f4", fontFamily: font }}>
      ...
      {/* ── 히어로 헤더 ── */}
      <div style={{
        background: post.bg, paddingTop: 72,
      ...
      <div style={{ maxWidth: 1120, margin: "0 auto", padding: "40px 24px 80px", display: "grid", gridTemplateColumns: "1fr 240px", gap: 40, alignItems: "start" }}>
        ...
        <div>
          ...
          >
            {/* 요약 */}
            <div style={{
              background: "#fdf8f6", borderLeft: `3px solid ${accent}`,
          ...
          <div style={{
            ...
            <div style={{ display: "flex", gap: 8 }}>
              {[
                ...
                { label: "카카오", icon: "💬", action: () => {} },
                { label: "트위터", icon: "🐦", action: () => {} },
              ].map(btn => (
                <button key={btn.label}
                  ...
                  onMouseEnter={e => { e.currentTarget.style.background = "#fff3f0"; e.currentTarget.style.borderColor = accent; }}
          ...
          <div style={{
            ...
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: 16, fontWeight: 700, color: "#191919", marginBottom: 4 }}>{post.author}</div>
              <div style={{ fontSize: 13, color: "#888", lineHeight: 1.5 }}>
                프론트엔드 개발자. 코드와 커피를 좋아합니다 ☕<br />
                React, Next.js, Python에 관심이 많습니다.
              </div>
            </div>
            <button
              onClick={() => onNavigate("home")}
          ...
          {relatedPosts.length > 0 && (
            <div style={{ marginTop: 36 }}>
              <h3 style={{ fontSize: 16, fontWeight: 700, color: "#191919", marginBottom: 16 }}>
                관련 글
              </h3>
              <div style={{ display: "flex", gap: 16 }}>
                {relatedPosts.map(rp => (
                  <RelatedPost key={rp.id} post={rp} onNavigate={onNavigate} />
                ))}
              </div>
            </div>
          )}
          ...
          <div style={{ display: "flex", gap: 12, marginTop: 24 }}>
            {posts[posts.findIndex(p => p.id === post.id) - 1] && (() => {
              const prev = posts[posts.findIndex(p => p.id === post.id) - 1];
              return (
                <button
                  onClick={() => onNavigate("post", prev.id)}
                  style={{
                ...
                >
                  <div style={{ fontSize: 11, color: "#bbb", marginBottom: 6 }}>← 이전 글</div>
            ...
            {posts[posts.findIndex(p => p.id === post.id) + 1] && (() => {
              const next = posts[posts.findIndex(p => p.id === post.id) + 1];
              return (
                <button
                  onClick={() => onNavigate("post", next.id)}
                  style={{
                    flex: 1, padding: "16px 20px",
                    background: "#fff", border: "1px solid #ece9e4",
                  ...
                  onMouseLeave={e => e.currentTarget.style.borderColor = "#ece9e4"}
                >
                  <div style={{ fontSize: 11, color: "#bbb", marginBottom: 6 }}>다음 글 →</div>
                  <div style={{ fontSize: 13, fontWeight: 600, color: "#333", lineHeight: 1.4,
                    display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical", overflow: "hidden" }}>
                    {next.title}
                  </div>
        ...
        <aside>
          ...
          <button
            ...
            onMouseLeave={e => { e.currentTarget.style.borderColor = "#ece9e4"; e.currentTarget.style.color = "#666"; }}
          >
            ← 목록으로
          </button>
        </aside>
      ...
      <style>{`
        ...
        .post-content strong { font-weight: 700; color: #191919; }
        .post-content em { font-style: italic; color: #555; }
        .post-content code {
        ...
        .post-content a { color: ${accent}; text-decoration: underline; }
      `}</style>
    </div>
  );
}
