...
function Step1Info({ form, setForm, onNext, onPrev }) {
    ...
    const checkEmail = val => {
        ...
        emailTimer.current = setTimeout(async () => {
            try {
                const response = await api.get(`/auth/check/email?email=${val}`);
                setEmailStatus(response.data.data.available ? "ok" : "dup");
    ...
    const checkNickname = val => {
        ...
        nicknameTimer.current = setTimeout(async () => {
            try {
                const response = await api.get(`/auth/check/nickname?nickname=${val}`);
                setNicknameStatus(response.data.data.available ? "ok" : "dup");
            } catch (e) { setNicknameStatus(null); }
        }, 600);
    };
...
/* ─── STEP 2 : 블로그 설정 ───────────────────────────── */
function Step2Blog({ form, setForm, agreed, onNext, onPrev }) {
    const [errors,     setErrors]     = useState({});
    ...
    const checkAddr = val => {
        ...
        addrTimer.current = setTimeout(async () => {
            try {
                const response = await api.get(`/auth/check/blog-address?blogAddress=${val}`);
                setAddrStatus(response.data.data.available ? "ok" : "dup");
            } catch (e) { setAddrStatus(null); }
        }, 700);
    };
    ...
    const handleComplete = async () => {
        ...
        // ── 백엔드 SignupRequest 전체 필드 매핑 ──
        // birthDate: "YYYY-MM-DD" 포맷으로 조합
        const birthYear  = String(form.birthYear  || "").padStart(4, "0");
        const birthMonth = String(form.birthMonth || "").padStart(2, "0");
        const birthDay   = String(form.birthDay   || "").padStart(2, "0");
        ...
        const payload = {
            ...
            blogAddress:      form.blogAddr,          // ← 필드명 수정: blogAddr → blogAddress
            blogDescription:  form.blogDesc   || "",
            profileEmoji:     form.profileEmoji || "👩‍💻",
            theme:            form.theme       || "light",

            // Step0 약관 동의 — @AssertTrue 검증 대상이므로 반드시 전송
            agreedService:    !!agreed.service,
            agreedPrivacy:    !!agreed.privacy,
            agreedLocation:   !!agreed.location,
            agreedMarketing:  !!agreed.marketing,
        };
    ...
    return (
        <div style={card}>
          ...
          <span style={{ padding: "0 12px", fontSize: 13, color: "#aaa", background: "#fafafa", borderRight: "1px solid #f0f0f0", height: 48, display: "flex", alignItems: "center", whiteSpace: "nowrap" }}>
            ...
            <div style={{ display: "flex", gap: 10, marginTop: 28 }}>
                <button type="button" onClick={onPrev} style={btnOutline}>이전</button>
                <button type="button" onClick={handleComplete} disabled={loading} style={btnPrimary(loading)}>
    ...
    return (
        <div style={{ minHeight: "100vh", background: "#f8f8f6", display: "flex", flexDirection: "column", fontFamily: "'Noto Sans KR','Apple SD Gothic Neo',sans-serif" }}>
            ...
            <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", padding: "40px 24px 60px" }}>
                ...
                {/* agreed를 Step2Blog에 전달하여 최종 제출 시 약관 동의 값 포함 */}
                {step === 2 && <Step2Blog  form={form}    setForm={setForm}      agreed={agreed} onNext={next} onPrev={prev} />}
